...
def measureBackgroundLightLevel(img):
    if DEBUG: print('>> in measureBackgroundLightLevel: measuring light level of image and estimate its average color')
    imgScaled = prepr.scaleImg(img, 30)
    # cv2.imshow('scaled', imgScaled)  # for TESTING
...
def getBrightnessContrastChange(lightLevel):
    if DEBUG: print('>> in getBrightnessContrastChange()')  
    brightnessChange = 0
    contrastChange = 0
    ...
    if 35 <= lightLevel and lightLevel <= 45:           # this is light level of real images in folder 15march
        ...
        if DEBUG: print('I do not recognize this light level')
        brightnessChange = 82
        contrastChange = 95
